const Game = {
  ...
  makeTimer: function() {
    this.timer = new Timer(this.ctx, this.backgroundMain.posX, this.backgroundMain.posY, 20000, this.gameW, this.gameH);
  },
  ...
  checkMonsterCollision() {
    this.monsters.some(monster => {
      if (
        monster.posX <= this.character.posX + this.character.width - 10 &&
        monster.posX + monster.width >= this.character.posX + 30 &&
        monster.posY <= this.character.posY + this.character.height + 32 &&
        monster.posY + monster.height >= this.character.posY
      ) {
  ...
  pauseLoop() {
    if (this.character.checkEnding()) {
      if (this.loop.volume > 0.05) {
        this.loop.volume -= 0.01;
      }
    } else {
      if (this.loop.volume < 0.31) {
        this.loop.volume += 0.01;
      }
    }
  },
